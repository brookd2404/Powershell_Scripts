...
try {
    Write-Output "Getting all Windows Managed Devices"
    $AllManagedDevices = @{
        Method = "GET"
        URI = "$GraphURL/deviceManagement/manageddevices"
        Headers = $Intune_Headers
        ContentType = "Application/JSON"
    }

    $ManagedDevices = Invoke-RestMethod @AllManagedDevices -ErrorAction Stop
    $global:All_ManagedDevices = @()
    $All_ManagedDevices += $ManagedDevices
    $Count = 1
    while(!([String]::IsNullOrEmpty($ManagedDevices.'@odata.nextLink'))) {
        Write-Output "Querying @odata.next link NO: $($Count)..."
        $ManagedDevices_NextLink = @{
            Method = "GET"
            URI = $ManagedDevices.'@odata.nextLink'
            Headers = $Intune_Headers
            ContentType = "application/JSON"
        }
        $ManagedDevices = Invoke-RestMethod @ManagedDevices_NextLink -ErrorAction Stop
        $All_ManagedDevices += $ManagedDevices
        $Count++
    }

    Write-Output "Managed Device Count $($All_ManagedDevices.Value.Count)"
} catch {
    Write-Output $Error[0]
    Throw "Failed to get Managed Devices"
}

$All_DeviceReturnValues = @()
FOREACH ($Device in ($All_ManagedDevices.value | Where-Object {$_.OperatingSystem -eq "Windows"})) {
    $MyInfo = @{
        Method = "GET"
        URI = "$GraphURL/deviceManagement/manageddevices('$($Device.id)')?`$select=id,hardwareinformation,devicename"
        Headers = $Intune_Headers
        ContentType = "application/JSON"
    }

    $Outcome = Invoke-RestMethod @MyInfo

    "OS Lang is $($Outcome.Hardwareinformation.Operatingsystemlanguage)"

    $Obj = New-Object PSObject 
    $Obj | Add-Member -MemberType NoteProperty -Name "id" -Value $Outcome.id
    $Obj | Add-Member -MemberType NoteProperty -Name "deviceName" -Value $Outcome.deviceName   
    $Obj | Add-Member -MemberType NoteProperty -Name "OSLanguage" -Value $Outcome.Hardwareinformation.Operatingsystemlanguage  

    $All_DeviceReturnValues += $Obj
}

$All_DeviceReturnValues | Export-Csv -NoClobber -NoTypeInformation C:\Temp\OS-Lang-Bruns2.csv -Force